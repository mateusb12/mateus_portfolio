...
import languageFile from "../data/contact.json";

export const Contact = () => {
  const {selectedFlag, setSelectedFlag} = useContext(LanguageContext);
  const languageFile = require('../data/contact.json');
  const languageKey = selectedFlag === 'usa' ? 'english' : 'portuguese';
  const textData = languageFile[languageKey];
              ...
              {({ isVisible }) =>
                ...
                <form onSubmit={handleSubmit}>
                  <Row>
                    ...
                    <Col size={12} sm={6} className="px-1">
                      <input type="text" value={formDetails.lasttName} placeholder={textData.surnameField} onChange={(e) => onFormUpdate('lastName', e.target.value)}/>
                    </Col>
                    <Col size={12} sm={6} className="px-1">
